...
-- ATUALIZAÇÃO:
-- 1. Mantém correções de CREATE TABLE.
-- 2. Mantém DROP para colunas erradas.
-- 3. ADICIONA lógica para preencher `balanceamento_curva` (Painel 1).

-- ... (CREATE/DROP Tables anteriores mantidos por segurança) ...

-- 1. Performance Entrega
DROP TABLE IF EXISTS dashboards.performance_entrega;
CREATE TABLE dashboards.performance_entrega (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    total_pedidos INT,
    dias_0 INT, perc_0 NUMERIC,
    dias_1 INT, perc_1 NUMERIC,
    dias_2 INT, perc_2 NUMERIC,
    dias_3 INT, perc_3 NUMERIC,
    dias_4 INT, perc_4 NUMERIC,
    dias_5 INT, perc_5 NUMERIC,
    acima_5 INT, perc_acima_5 NUMERIC
);

-- 2. Resumo Por Item (Curva ABC Itens)
DROP TABLE IF EXISTS dashboards.resumo_por_item;
CREATE TABLE dashboards.resumo_por_item (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    it_codigo TEXT,
    desc_item TEXT,
...
-- 3. Pedidos Recebidos
DROP TABLE IF EXISTS dashboards.pedidos_recebidos;
CREATE TABLE dashboards.pedidos_recebidos (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    data DATE,
    qtd_ped INT
...
-- 4. Vendas Diárias
DROP TABLE IF EXISTS dashboards.vendas_diaria;
CREATE TABLE dashboards.vendas_diaria (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    data DATE,
    valor NUMERIC
        ...
        IF v_bal IS NOT NULL THEN
            ...
            FOREACH v_c IN ARRAY v_curva LOOP
                ...
                INSERT INTO dashboards.balanceamento_curva (
                    curva, d0_15, d15_30, d15_60, d60_120, d0_120, total
                ) VALUES (
                    v_c,
                    COALESCE(NULLIF(v_bal->'ate_15_dias'->>v_c, '')::INT, 0),
                    COALESCE(NULLIF(v_bal->'15_a_30_dias'->>v_c, '')::INT, 0),
