-- Revert Schema Rename: dashboards_pcp -> dashboards
-- Reason: API Configuration does not allow 'dashboards_pcp' yet.

DO $$
BEGIN
    -- 1. Rename Schema Back
    IF EXISTS(SELECT 1 FROM information_schema.schemata WHERE schema_name = 'dashboards_pcp') THEN
        ALTER SCHEMA dashboards_pcp RENAME TO dashboards;
    END IF;

    -- 2. Update Function (Revert References)
    -- We need to drop the function in the "old" (current) schema name first if created there? 
    -- Actually, if we rename schema, the function inside moves with it.
    -- But the function BODY text still refers to 'dashboards_pcp'. We must update it.
END
$$;

-- Update function body to use 'dashboards'
...
BEGIN
    ...
    IF v_painel1 IS NOT NULL THEN
        ...
        IF (v_painel1->'cardsPedidos') IS NOT NULL THEN
            DELETE FROM dashboards.cards_pedidos WHERE 1=1;
            INSERT INTO dashboards.cards_pedidos (total, conferir, pendente, emitir_nf, expedicao, frete_faf, programado, chegou_hoje)
            SELECT 
        ...
        IF (v_painel1->'estoqueEstrategico') IS NOT NULL THEN
            DELETE FROM dashboards.estoque_produtos_estrategicos WHERE 1=1;
        ...
        IF (v_painel1->'estoqueAcabado') IS NOT NULL THEN
            DELETE FROM dashboards.balanceamento_estoque_acabado WHERE 1=1;
            INSERT INTO dashboards.balanceamento_estoque_acabado (estoque_total, carteira_pedidos, estoque_disponivel)
            SELECT 
        ...
        IF (v_painel1->'performanceEntrega') IS NOT NULL THEN 
            DELETE FROM dashboards.performance_entrega WHERE 1=1;
        ...
        IF (v_painel1->'resumoPorItem') IS NOT NULL THEN 
            DELETE FROM dashboards.resumo_por_item WHERE 1=1;
        ...
        IF (v_painel1->'historicoPendencia') IS NOT NULL THEN 
            DELETE FROM dashboards.historico_pedidos WHERE 1=1;
        ...
        IF (v_painel1->'pedidosRecebidosSemana') IS NOT NULL THEN 
            DELETE FROM dashboards.pedidos_recebidos WHERE 1=1;
        ...
        IF v_bal IS NOT NULL THEN
            DELETE FROM dashboards.balanceamento_curva WHERE 1=1;
            ...
            FOREACH v_c IN ARRAY v_curva LOOP
                ...
                INSERT INTO dashboards.balanceamento_curva (
    ...
    IF v_painel2 IS NOT NULL THEN
        IF (v_painel2->'estoqueFundicao') IS NOT NULL THEN
            DELETE FROM dashboards.estoque_fundicao WHERE 1=1;
            INSERT INTO dashboards.estoque_fundicao (estoque_aluminio, estoque_fundido, recebido_aluminio, recebido_fundido)
            SELECT 
        ...
        IF (v_painel2->'producaoMaquinas') IS NOT NULL THEN
            DELETE FROM dashboards.media_prensa_injetora WHERE 1=1;
            INSERT INTO dashboards.media_prensa_injetora (prensa_rei, injetora_rei, prensa_rubber, injetora_rubber)
            SELECT 
        ...
        IF (v_painel2->'comparativoMensal') IS NOT NULL THEN
            DELETE FROM dashboards.acompanhamento_diario WHERE 1=1;
    ...
    IF v_painel3 IS NOT NULL THEN
        IF (v_painel3->'carteiraME') IS NOT NULL THEN
            DELETE FROM dashboards.carteira_me WHERE 1=1;
            INSERT INTO dashboards.carteira_me (sem_saldo, nao_alocado, faf_impressa, nao_efetivado, alocad_parcial, embarque_criado, pesagem_realizada, frete_analisado, pedido_separado, pedido_embalando_total, pedido_embalando_parcial)
            SELECT 
        ...
        IF (v_painel3->'carteiraMI') IS NOT NULL THEN
            DELETE FROM dashboards.carteira_mi WHERE 1=1;
            INSERT INTO dashboards.carteira_mi (sem_saldo, nao_alocado, faf_impressa, nao_efetivado, alocad_parcial, embarque_criado, pesagem_realizada, frete_analisado, pedido_separado, pedido_embalando_total, pedido_embalando_parcial)
            SELECT 
        ...
        IF (v_painel3->'faturamentoDiario') IS NOT NULL THEN
            DELETE FROM dashboards.faturamento_diario WHERE 1=1;
        ...
        IF (v_painel3->'vendasDiarias') IS NOT NULL THEN
            DELETE FROM dashboards.vendas_diaria WHERE 1=1;
