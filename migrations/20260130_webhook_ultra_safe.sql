-- Versão ULTRA-SEGURA: Remove todos os UPDATEs genéricos
-- Usa INSERT ... ON CONFLICT para evitar problemas com políticas RLS
...
BEGIN
    ...
    BEGIN
        ...
        -- Tenta salvar sucesso (mas ignora erro se webhook_logs tiver RLS strict)
        BEGIN
            IF EXISTS (SELECT 1 FROM public.webhook_logs WHERE id = NEW.id) THEN
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ...
            -- Tenta salvar erro (mas ignora se webhook_logs tiver RLS strict)
            BEGIN
                IF EXISTS (SELECT 1 FROM public.webhook_logs WHERE id = NEW.id) THEN
                    UPDATE public.webhook_logs 
