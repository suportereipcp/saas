-- PASSO 1: Remove a função antiga
DROP FUNCTION IF EXISTS public.process_dashboards_webhook() CASCADE;

-- PASSO 2: Cria função nova usando DELETE FROM
CREATE OR REPLACE FUNCTION public.process_dashboards_webhook()
RETURNS TRIGGER AS $$
DECLARE
    v_payload JSONB;
    v_painel1 JSONB;
    ...
    -- Variáveis para Balanceamento Curva
    v_bal JSONB;
    v_val_d60 INT;
    v_val_d90 INT;
...
BEGIN
    ...
    -- Bloco de processamento com tratamento de erros
    BEGIN
        ...
        IF v_painel1 IS NOT NULL THEN
            ...
            IF (v_painel1->'cardsPedidos') IS NOT NULL THEN
                DELETE FROM dashboards_pcp.cards_pedidos WHERE TRUE;
            ...
            IF (v_painel1->'estoqueEstrategico') IS NOT NULL THEN
                DELETE FROM dashboards_pcp.estoque_produtos_estrategicos WHERE TRUE;
            ...
            IF (v_painel1->'estoqueAcabado') IS NOT NULL THEN
                DELETE FROM dashboards_pcp.balanceamento_estoque_acabado WHERE TRUE;
            ...
            IF (v_painel1->'performanceEntrega') IS NOT NULL THEN 
                DELETE FROM dashboards_pcp.performance_entrega WHERE TRUE;
            ...
            IF (v_painel1->'resumoPorItem') IS NOT NULL THEN 
                DELETE FROM dashboards_pcp.resumo_por_item WHERE TRUE;
            ...
            IF (v_painel1->'historicoPendencia') IS NOT NULL THEN 
                DELETE FROM dashboards_pcp.historico_pedidos WHERE TRUE;
            ...
            IF (v_painel1->'pedidosRecebidosSemana') IS NOT NULL THEN 
                DELETE FROM dashboards_pcp.pedidos_recebidos WHERE TRUE;
            ...
            IF v_bal IS NOT NULL THEN
                DELETE FROM dashboards_pcp.balanceamento_curva WHERE TRUE;
        ...
        IF v_painel2 IS NOT NULL THEN
            IF (v_painel2->'estoqueFundicao') IS NOT NULL THEN
                DELETE FROM dashboards_pcp.estoque_fundicao WHERE TRUE;
                ...
                SELECT 
                    ...
                    COALESCE(NULLIF(v_painel2->'estoqueFundicao'->>'recebidoFerro', '')::INT, 0); 
            END IF;
            
            IF (v_painel2->'producaoMaquinas') IS NOT NULL THEN
                DELETE FROM dashboards_pcp.media_prensa_injetora WHERE TRUE;
            ...
            IF (v_painel2->'comparativoMensal') IS NOT NULL THEN
                DELETE FROM dashboards_pcp.acompanhamento_diario WHERE TRUE;
        ...
        IF v_painel3 IS NOT NULL THEN
            IF (v_painel3->'carteiraME') IS NOT NULL THEN
                DELETE FROM dashboards_pcp.carteira_me WHERE TRUE;
            ...
            IF (v_painel3->'carteiraMI') IS NOT NULL THEN
                DELETE FROM dashboards_pcp.carteira_mi WHERE TRUE;
            ...
            IF (v_painel3->'faturamentoDiario') IS NOT NULL THEN
                DELETE FROM dashboards_pcp.faturamento_diario WHERE TRUE;
            ...
            IF (v_painel3->'vendasDiarias') IS NOT NULL THEN
                DELETE FROM dashboards_pcp.vendas_diaria WHERE TRUE;
        ...
        -- Log de sucesso (apenas nos logs do PostgreSQL)
        RAISE NOTICE 'Webhook processado com sucesso - ID: %', NEW.id;

    EXCEPTION
        WHEN OTHERS THEN
            -- Captura erro e loga
            v_error_msg := SQLERRM;
            RAISE WARNING 'Erro ao processar webhook ID %: %', NEW.id, v_error_msg;
            
            -- Continua sem falhar o trigger
            RETURN NEW;
    END;
