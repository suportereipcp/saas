    ...
    const saveChanges = async () => {
        try {
            ...
            allDates.forEach(date => {
                ...
                // Map to DB columns
                // status 'off' -> type 'feriado'
                // status 'work' -> type 'dia_util'
                // status 'half' -> type 'meio_dia'
                let dbType = null;
                ...
                if (dbType || obs) {
                    // Only insert if we have a type (exception) or a comment
                    // But if it's just a comment on a normal day, we explicitly insert validation?
                    // Original logic: if (status || obs). 
                    // If no status logic (normal day) but has obs, we need a type? 
                    // DB constraint requires type? No, type check is NOT NULL in my script? Yes "type TEXT NOT NULL".
                    // So if it's just a comment, what type is it?
                    // If no exception, it matches default logic (work or off).
                    // If default is Work, and we comment, we must insert 'work' (dia_util) or handle null?
                    // Let's assume for now we only support comments on Exceptions or we force the type to current default?

                    // Simple fix: If no exception type, define it based on isWeekend?
                    if (!dbType && obs) {
                        const d = new Date(date + 'T12:00:00'); // Safe date parse
                        dbType = isWeekend(d) ? 'feriado' : 'dia_util'; // Default fallback
                        // Actually if it's weekend and NO exception, it's 'feriado' (off).
                        // If it's weekday and NO exception, it's 'dia_util'.
                    }
