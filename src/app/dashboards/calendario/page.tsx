    ...
    const loadCalendar = async () => {
        try {
            ...
            data?.forEach((row: any) => {
                if (row.status === 'off') dbHolidays.push(row.data);
                if (row.status === 'work') dbCustomWork.push(row.data);
                if (row.status === 'half') dbHalf.push(row.data);
                if (row.obs) dbComments[row.data] = row.obs;
            });
            ...
            console.error("Erro ao carregar calendário:", JSON.stringify(error, null, 2) || error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const saveChanges = async () => {
        try {
            ...
            allDates.forEach(date => {
                ...
                // We only persist if there is a status exception OR a comment
                if (status || obs) {
                    records.push({ data: date, status, obs });
                }
            });
        ...
        <div className="flex flex-col h-full w-full p-4 gap-4 font-sans text-foreground overflow-hidden relative">
            ...
            <div className="flex items-center justify-between bg-card p-3 rounded-2xl border border-border backdrop-blur-md shadow-lg shrink-0">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold tracking-wide text-[#374151]">Definição de Calendário</h1>
                    </div>
                </div>
                {/* Calendar Controls */}
